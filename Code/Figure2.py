...
# NEW: For the three CoCo panels, any unique site/date observation < 2.7 mm is
# set to 0.0 mm, and the corresponding forecast for every model containing that
# site/date case is also set to 0.0 mm before daily means and accumulations.
#
# -----------------------------------------------------------------------------
# User settings
# -----------------------------------------------------------------------------
...
OUTPUT_FIG = OUTPUT_DIR / "CoCo_SS_running_accumulation_5panel_v16_CoCo_LT2p7_zeroed_20241115_20250401.png"
...
# CoCo-only threshold treatment.
COCO_ZERO_THRESHOLD_MM = 2.7
...
def apply_coco_zero_threshold(
	model_rows: Dict[str, pd.DataFrame],
	threshold_mm: float,
) -> None:
	"""For CoCo, zero observations < threshold and all corresponding forecasts."""
	observation_frames = []
	...
	low_cases = canonical_obs[
		canonical_obs["observed_mm"] < threshold_mm
	][["date", "site"]]
	low_case_index = pd.MultiIndex.from_frame(low_cases)

	print(
		f"CoCo threshold: {len(low_case_index):,} unique date/site cases with "
		f"observed < {threshold_mm:.1f} mm will be forced to 0.0 mm"
	)

	for model_name, df in model_rows.items():
		row_index = pd.MultiIndex.from_frame(df[["date", "site"]])
		mask = row_index.isin(low_case_index)
		n_rows = int(mask.sum())

		df.loc[mask, "observed_mm"] = 0.0
		df.loc[mask, "forecast_mm"] = 0.0

		print(
			f"{model_name}: zeroed observed and forecast for {n_rows:,} rows"
		)
...
def load_panel(config: Dict) -> pd.DataFrame:
	...
	if config.get("apply_coco_zero_threshold", False):
		apply_coco_zero_threshold(model_rows, COCO_ZERO_THRESHOLD_MM)
